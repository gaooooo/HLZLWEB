...
export default class {
    ...
    getConnection() {
            ...
            if (this.isOnce || !this.ftpConnection) {
                connection = this.ftpConnection = new PromiseFtp();
            }
            if (this.getStatus(connection) === FTP_STATE.disconnected) {
                connectionPromise = this.ftpConnection.reconnect(this.ftpConfig);
            }
            if (!connectionPromise) {
                connectionPromise = this.ftpConnection.connect(this.ftpConfig);
            }
    ...
    resolveRemotePath(remotePath, isAbsolute) {
            ...
            return this.pathResolve(prefix + remotePath);
        }
        //关闭连接
    closeConnection(connection) {
            return (connection || this.ftpConnection).end().then(() => {
